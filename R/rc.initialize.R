rc.initialize=function(cyto.info,num.tracks=NULL,chr.order=NULL,stepUnit=10^7,Layout=c('circular','landscape'),params=list()){
	...
	if(!is.null(num.tracks)){
		...
		if(num.tracks > params$default.tracks){
			...
			if(params$Layout=='circular') params$radius = params$radius + params$track.height * (1+params$track.padding) * (num.tracks - params$default.tracks)
			if(params$Layout=='landscape'){
				params$track.height = params$radius / params$num.tracks / (1+params$track.padding)
			}
		}
	}
	#
